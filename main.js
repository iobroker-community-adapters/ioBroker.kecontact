...
class Kecontact extends utils.Adapter {
    ...
    stateWallboxDisabled     = 'automatic.pauseWallbox';      /*switch to generally disable charging of wallbox, e.g. because of night storage heater */
    statePvAutomatic         = 'automatic.photovoltaics';     /*switch to charge vehicle in regard to surplus of photovoltaics (false= charge with max available power) */
    ...
    stateMinimumSoCOfBatteryStorage = 'automatic.batterySoCForCharging'; /*SoC above which battery storage may be used for charging vehicle*/
    ...
    async onReady() {
        ...
        this.log.info('config.1p3pSwitchIsNO: ' + this.config['1p3pSwitchIsNO'] +
        ', 1p = ' + this.valueFor1pCharging + ', 3p = ' + this.valueFor3pCharging + ', off = ' + this.valueFor1p3pOff);
    ...
    onUnload(callback) {
        try {
            ...
            try {
                ...
                if (this.log) {   // got an exception 'TypeError: Cannot read property 'warn' of undefined'
    ...
    detectIoBrokerLanguage() {
        const adapter = this;
        this.getForeignObject('system.config', function(err, ioBroker_Settings) {
            if (err) {
    ...
    checkConfig() {
        ...
        if (this.photovoltaicsActive) {
            ...
            if (! this.config.minTime || this.config.minTime < 0) {
                this.log.info('minimum charge time not speficied or too low, using default value of ' + this.minChargeSeconds);
            } else {
                this.minChargeSeconds = this.getNumber(this.config.minTime);
            }
            if (! this.config.regardTime || this.config.regardTime < 0) {
                this.log.info('minimum regard time not speficied or too low, using default value of ' + this.minRegardSeconds);
            } else {
    ...
    init1p3pSwitching(stateNameFor1p3p) {
        ...
        this.getForeignState(stateNameFor1p3p, function (err, obj) {
            if (err) {
    ...
    handleWallboxMessage(message, remote) {
        ...
        if (this.isMessageFromWallboxOfThisInstance(remote)) {     // handle only message from wallbox linked to this instance, ignore other wallboxes sending broadcasts
                ...
                if (this.states[sessionid + '_' + key] || this.loadChargingSessions === false) {
                    try {
                        if (message.ID == 100) {
                            ...
                            switch (key) {
                                case 'Session ID': this.setStateAck(this.stateSessionId, message[key]); break;
                                case 'RFID tag': this.setStateAck(this.stateRfidTag, message[key]); break;
                                case 'RFID class': this.setStateAck(this.stateRfidClass, message[key]); break;
                            }
                        }
                        if (this.loadChargingSessions) {
    ...
    getWallboxPowerInWatts() {
        if (this.getWallboxType() == this.TYPE_D_EDITION) {
            if (this.isVehiclePlugged() && (this.getStateDefault0(this.stateWallboxState) == 3)) {
                return this.getStateDefault0(this.stateWallboxCurrent) * this.voltage * this.getChargingPhaseCount() / 1000;
            } else {
                return 0;
            }
        } else {
    ...
    getBatteryStoragePower(isFullPowerRequested) {
        ...
        const batteryPower = this.getStateDefault0(this.config.stateBatteryCharging) - this.getStateDefault0(this.config.stateBatteryDischarging);
    ...
    getSurplusWithoutWallbox(isFullBatteryStoragePowerRequested = false) {
        let power = this.getStateDefault0(this.config.stateSurplus)
     ...
     */
    getTotalPower() {
        let result = this.getStateDefault0(this.config.stateEnergyMeter1)
        ...
        if (newValue !== null) {
            if (this.isX2PhaseSwitch()) {
                if (newValue != this.getStateDefault0(this.stateX2Switch)) {
                    ...
                    this.log.info('updating X2 for switch of phases from ' + this.getStateDefault0(this.stateX2Switch) + ' to ' + newValue + '...');
    ...
    getCurrentForSwitchTo3p() {
        return this.getMinCurrent() * this.get1p3pPhases() * 1.10;
    }
    ...
    get1p3pPhases() {
        if (this.isReducedChargingBecause1p3p()) {
            ...
            if (this.isVehicleCharging() && phases > 1 && this.getChargingPhaseCount() > 1) {
                this.log.error('Charging with ' + phases + ' but reduced (1p) expected, disabling 1p/3p switch for this charging session');
                this.reset1p3pSwitching();
                this.stepFor1p3pSwitching = -1;
            }
            if (phases <= 0) {
     ...
     */
    isVehiclePlugged(myValue) {
        let value;
        if (myValue) {
            value = myValue;
        } else {
            value = this.getStateInternal(this.stateWallboxPlug);
        }
        // 0 unplugged
    ...
    check1p3pSwitchingRetries() {
        if (this.retries1p3pSwitching >= 3) {
            this.log.error('switching not possible in step ' + this.stepFor1p3pSwitching + ', disabling 1p/3p switch for this charging session');
            this.reset1p3pSwitching();
            this.stepFor1p3pSwitching = -1;
            return true;
        }
                ...
                if (chargeTimestamp !== null) {
                    if (curr < this.getMinCurrent()) {
                        ...
                        if (this.underusage > 0) {
                            this.log.debug('check with additional power of: ' + addPower + ' and underUsage: ' + this.underusage);
                    ...
                    if (curr < this.getMinCurrent()) {
                        if (this.isContinueDueToMinChargingTime(newDate, chargeTimestamp)) {
                            this.log.info('minimum charge time of ' + this.minChargeSeconds + 'sec not reached, continuing charging session. ' + chargeTimestamp);
                            curr = this.getMinCurrent();
                    ...
                    if (curr < this.getMinCurrent()) {
                        if (this.isContinueDueToMinRegardTime(newDate)) {
                            this.log.info('minimum regard time of ' + this.minRegardSeconds + 'sec not reached, continuing charging session. RegardTimestamp: ' + regardTimestamp);
                            curr = this.getMinCurrent();
                            newValueFor1p3pSwitching = null;  // than also stop possible 1p/3p switching
                        }
                    } else {
        ...
        switch (this.getWallboxModel()) {
            ...
            case this.MODEL_P20:
                switch (type.substr(13,1)) {
                    case '0': return this.TYPE_E_SERIES;
                    case '1':
            ...
            case this.MODEL_P30:
                if (type.endsWith('-DE')) {   // KEBA says there's only one ID: KC-P30-EC220112-000-DE
                    this.sendWallboxWarning('Keba KeContact P30 Deutschland-Edition detected. Regulation may be inaccurate.');
                ...
                if (block) {
                    ...
                    if (currFirmwareList) {
                        ...
                        if (block[1] == currFirmwareList[1]) {
                            ...
                            this.log.warn(prefix + 'current firmware ' + currFirmwareList[1] + ', <a href="' + this.firmwareUrl + '">new firmware ' + block[1] + ' available</a>');
                        }
                    } else {
